    ...
    const std::function<void()>                              &throw_on_cancel_callback)
{
    ...
    const bool is_mm_painted = std::any_of(model_volumes.cbegin(), model_volumes.cend(), [](const ModelVolume *mv) { return !mv->mmu_segmentation_facets.empty(); });
...
void PrintObject::slice_volumes()
{
    ...
    if (const auto& volumes = this->model_object()->volumes;
        ...
        // Do we have also enabled XY Size compensation that would not be used because the object is MM painted?
    ...
    BOOST_LOG_TRIVIAL(debug) << "Slicing volumes - make_slices in parallel - begin";
    {
        ...
        const bool  is_mm_painted                     = std::any_of(this->model_object()->volumes.cbegin(), this->model_object()->volumes.cend(), [](const ModelVolume *mv) { return !mv->mmu_segmentation_facets.empty(); });
        const auto  xy_compensation_scaled            = is_mm_painted ? scaled<float>(0.f) : scaled<float>(std::min(m_config.xy_size_compensation.value, 0.));
